var tactics=[];(function(){function Tablesort(el,options){if(!(this instanceof Tablesort))return new Tablesort(el,options);if(!el||el.tagName!=='TABLE'){throw new Error('Element must be a table')}
...
res.data.forEach(element=>{element.Formation=element.Formation.split(',').join('-').split('.').join('-').split(' ')[0]
