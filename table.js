        ...
        init: function(el, options) {
            ...
            for (i = 0; i < firstRow.cells.length; i++) {
                ...
                if (cell.getAttribute('data-sort-method') !== 'none') {
                    cell.tabindex = 0;
                    cell.addEventListener('click', onClick, !1);
        ...
        sortTable: function(header, update) {
            ...
            for (i = 0; i < that.table.tBodies.length; i++) {
                ...
                if (sortOrder === 'descending') {
                    newRows.sort(stabilize(sortFunction, !0))
                } else {
                    newRows.sort(stabilize(sortFunction, !1));
                    newRows.reverse()
                }
...
(function() {
    ...
    Tablesort.extend('number', function(item) {
        return item.match(/^[-+]?[£\x24Û¢´€]?\d+\s*([,\.]\d{0,2})/) || item.match(/^[-+]?\d+\s*([,\.]\d{0,2})?[£\x24Û¢´€]/) || item.match(/^[-+]?(\d)*-?([,\.]){0,1}-?(\d)+([E,e][\-+][\d]+)?%?$/)
    }, function(a, b) {
        a = cleanNumber(a);
        b = cleanNumber(b);
